...
public class ListSelector : GameObject2D
{
    ...
    public bool IsPressed => _container.IsPressed;

    public float Height => _container.Height;

    public float Width => _container.Width;
    ...
    public void SetItems(List<IGameObject> items, int itemSize)
    {
        ...
        foreach (var itemObject in items)
        {
            ...
            item.Initialize();
            item.LoadContent(SceneManager.SceneManager.RenderContext.GlobalContentManager, false);
