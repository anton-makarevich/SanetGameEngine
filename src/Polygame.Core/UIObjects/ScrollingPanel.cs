...
public class ScrollingPanel :GameObject2D
{
	...
	public bool IsPressed => _touchProvider.Touch.IsPressed;
	...
	public int ChildrenCount => _container.Children.Count;

	public List<GameObject2D> PanelChildren => _container.Children;
	...
	public int CurrentStep => InnerCurrentStep *-1;

	public int MinStep 
	{
		get => _minStep * -1;
		set => _minStep = value * -1;
	}
	public int MaxStep 
	{
		get => _maxStep * -1;
		private set => _maxStep = value * -1;
	}
	...
	public Vector2 ContainerPosition => _container.LocalPosition;
	...
	public override void LoadContent(ContentManager contentManager,bool isLocal)
	{
		...
		{
			for (var i = 1; i <= _container.Children.Count; i++)
			{
	...
	public float ScrollingIndex => (ScrollingDirection== GameObjectOrientation.Vertical)?Height/ScrollingHeigth:Width/ScrollingWidth;
			...
			else
			{
				...
				if (IsFixedStep)
				{
					...
					if (ScrollingDirection == GameObjectOrientation.Vertical)
					{
						...
						{
							var closest = (float)Math.Round((_loc.Y+_totalDistance.Y) / ItemHeightOverride, 0);
					...
					else if (ScrollingDirection == GameObjectOrientation.Horizontal)
					{
						...
						{
							var closest = (float)Math.Round((_loc.X +_totalDistance.X)/ ItemWidthOverride, 0);
							InnerCurrentStep = (int)closest;
