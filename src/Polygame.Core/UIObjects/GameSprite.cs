...
public class GameSprite : GameObject2D
{
    ...
    { 
        ...
        set => _applyColorToChildren=value;
    }

    public string AssetFile => _assetFile;
    ...
    public override void LoadContent(ContentManager contentManager, bool isLocal)
    {
        ...
        if (IsCustomContent)
        {
            contentManager = SceneManager.SceneManager.RenderContext.GetContentManager(Path.Combine(SceneManager.SceneManager.RenderContext.GlobalContentManager.RootDirectory, "Scenes", CustomContent));
