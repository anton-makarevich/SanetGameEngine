...
public static class Extensions
{
    public static Rectangle Update(this Rectangle rectangle, Matrix transform, Vector2 deviceScale)
    {
        ...
        var width = (int)(newMax.X - newMin.X);
        var height = (int)(newMax.Y - newMin.Y);
    ...
    {
        var a = (float)color.A / 255f;
        var r = (float)color.R / 255f / a;
        var g = (float)color.G / 255f / a;
        var b = (float)color.B / 255f / a;
    ...
    public static void OnTextChange(this IKeyboardTracker tracker)
    {
        //firs check if new text is longer or shorter
        if (tracker.Text.Length > tracker.PrevText.Length)
        {
            //if longer - we need to add chars
            var dL = tracker.Text.Length - tracker.PrevText.Length; //difference in length, usually 1, but maybe more if user uses auto fill or other soft keyboard helpers
            var curChar = '\0';
            var curIndex = 0;
            //searching text to find difference
            for (var i = 0; i < tracker.Text.Length; i++)
            {
                curIndex = i;
            ...
            if (curChar != '\0')
            {
                ...
                for (var i = curIndex; i < (curIndex + dL); i++)
                {
        ...
        else if (tracker.Text.Length < tracker.PrevText.Length)
        {
            ...
            var dL = tracker.PrevText.Length - tracker.Text.Length; //how much chars were deleted
