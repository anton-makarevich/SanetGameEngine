...
/// </summary>
public class TextBlock : GameObject2D, ITextElement
{
    #region Constructor
    public TextBlock(string spriteFontName)
    {
    ...
    public void ProcessText()
    {
        ...
        foreach (var text in textsForLines)
            _lines.Add(new TextLine(_spriteFont, text));
        ...
        foreach (var line in _lines)
        {
            ...
            lineCount++;
            line.DrawSprite(SceneManager.SceneManager.RenderContext, TextColor);
            line.RenderedSprite.Translate(pos);
            line.RenderedSprite.Scale(FontScale*line.LocalScale);
            if (lineCount == _lines.Count)
                LinePosition = pos;

            AddChild(line.RenderedSprite);
    ...
    private Rectangle ProcessLines()
    {
        ...
        while (index < _lines.Count)
        {
            ...
            if (TextWrap && line.Size.X * FontScale.X > Rect.Width)
            {
                ...
                var i = Math.Max(lineText.LastIndexOf(" "),lineText.LastIndexOf("-"));
                ...
                if (i != -1)
                {
                    ...
                    {
                        _lines.Add(new TextLine(_spriteFont,  lastword));
                        lineInserted = true;
                    }
                    else
                    {
                        ...
                        {
                            _lines.Insert(index + 1, new TextLine(_spriteFont, lastword));
                            lineInserted = true;
                        }
                    }
        ...
        if (_lines.Count > 1)
            bounds.Height = LineSpacing*_lines.Count;//size.Y - 1;
        else
            bounds.Height = (int)_lines[0].Size.Y;
...
#if DEBUG
        if (Text == "Add Friends" && _c != TextColor)
        {
            var l = _lines[0];
            Debug.WriteLine(string.Format("Textblock Line Position LX:{0}, LY:{1}; Color:{2}", l.RenderedSprite.WorldPosition.X, l.RenderedSprite.WorldPosition.Y, TextColor.ToHexString()));
            _c = TextColor;
        }
#endif
    }
