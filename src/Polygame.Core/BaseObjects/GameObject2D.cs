...
namespace Sanet.Polygame.BaseObjects;
...
public class GameObject2D:IGameObject
{
    ...
    {
        get => _deviceScale;
        private set => _deviceScale = value;
    }
    ...
    public virtual void Update(RenderContext renderContext)
    {
        if (CanDraw || ForceUpdate)
        {
            ...
            var inWP = new Vector2(pos.X, pos.Y) * _deviceScale;
            ...
            if (WorldRotation != 0)
            {
                ...
                var scCP = PivotPoint * WorldScale/parentScale; //pivot point with rotation
                //points to rotate
                ...
                var cornersRotated = new Vector2[2];
                //rotation matrix
