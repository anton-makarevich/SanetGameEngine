...
public class PathAnimation : AnimationBase
{
    ...
    public override void Update(RenderContext renderContext)
    {
        ...
        if (IsPlaying && !IsPaused)
        {
            ...
            var delta = _speed * renderContext.GameTime.ElapsedGameTime.Milliseconds / 1000;

            var dD = (delta > dist) ? dist : delta;

            var dRot = (np.Rotation > _rotation) ? (int)np.Rotation - (int)_rotation : (int)_rotation - (int)np.Rotation;
